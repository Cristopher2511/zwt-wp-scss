...
class Wp_Scss {
 ...
 /**
  ...
  public function compile() {
      ...
      $input_files = $this->array_flatten($this->listDirectory($this->scss_dir));

      foreach ($input_files as $scss_file) {
        $outputlink = preg_replace("/\.[^$]*/",".css", $scss_file);
        $outputlink = str_replace('/scss/', '/css/', $outputlink);
        compiler($scss_file, $outputlink, $this);
      }
 ...
 function array_flatten($array) { 
  ...
  function listDirectory($path)
  {
    $filepaths = array(); 
      ...
      while (false !== ($file = readdir($handle))) {
          ...
          if ( is_dir($path.$file)) {
            $filepaths[] = $this->listDirectory($path.$file);
          } else {
            $ext = pathinfo($path.'/'.$file, PATHINFO_EXTENSION);
            if($ext=='scss'){
              $filepaths[] = preg_replace('/(\/+)/','/',$path.'/'.$file);;
            }
          } 
      }
      closedir($handle);
  ...
  //$newfilearray = array_flatten(listDirectory($wpscss_settings['scss_dir']));
  /**
